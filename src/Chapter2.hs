...
takeEven l = go [] 0 l
  where
    ...
    go acc counter (x:xs)
      | mod counter 2 == 0 = go (x:acc) (counter+1) xs
      | otherwise = go acc (counter+1) xs
...
contains find ys = go [] ys
  where
    go acc [] = reverse acc
    go acc (xs:xss)
      | elem find xs = go (xs:acc) xss
      | otherwise = go acc xss
...
rotate n l
  | n < 0 = []
  | otherwise = subList n (n + len) rotateEnough
    where
      len = length l
      rotateEnough = take (len + n) (cycle l)
